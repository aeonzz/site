...
export function Scrollspy({
  ...
  const setActiveSection = useCallback(
    (sectionId: string | null, force = false, updateHistory = true) => {
      if (!sectionId) return;
      ...
      if (
        updateHistory &&
        history &&
        (force || prevIdTracker.current !== sectionId)
      ) {
  ...
  const handleScroll = useCallback((updateHistory = true) => {
    ...
    // Find the last heading that has crossed into the upper viewport.
    let activeIdx = 0;
    ...
    anchorElementsRef.current.forEach((anchor, idx) => {
      ...
      const activationOffset = Math.min(64, viewportHeight * 0.12);
      const activationLine = scrollRect.top + customOffset + activationOffset;
    ...
    setActiveSection(sectionId, false, updateHistory);
  ...
  const scrollToHashSection = useCallback(() => {
    const hash = escapeAttributeValue(window.location.hash.replace("#", ""));
